...
class AirConditionerAccessory extends BaseAccessory {
  ...
  constructor(...props) {
    ...
    // команды режимов из конфига или дефолтные
    ...
    // совместимость со старыми числовыми скоростями
  ...
  _registerPlatformAccessory() {
    ...
    if (this.useThermostatUI) {
      ...
      // главный сервис для красивой группировки
      if (s.setPrimaryService) s.setPrimaryService(true);
    } else {
  ...
  _registerCharacteristics(dps) {
    ...
    this.dpTempUnits            = this._getCustomDP(this.device.context.dpTempUnits)          || null; // у многих нет
    ...
    if (this.useThermostatUI) {
      let service = this.accessory.getService(Service.Thermostat)
        || this.accessory.addService(Service.Thermostat, this.device.context.name);
      if (service.setPrimaryService) service.setPrimaryService(true);
      ...
      service.getCharacteristic(Characteristic.CurrentHeatingCoolingState)
        ...
        .on('get', (cb) => this.getState([this.dpActive, this.dpMode, this.dpCurrentTemperature, this.dpThreshold], (e, st) => cb(e, e ? undefined : this._hkCurrentFromDps(st))));

      // Units: если DP нет — всегда CELSIUS, ничего не запрашиваем
      service.getCharacteristic(Characteristic.TemperatureDisplayUnits)
        .updateValue(this._getTemperatureDisplayUnits(this.dpTempUnits ? dps[this.dpTempUnits] : undefined))
        .on('get', (cb) => {
          if (!this.dpTempUnits) return cb(null, Characteristic.TemperatureDisplayUnits.CELSIUS);
          this.getState(this.dpTempUnits, (err, dp) => cb(err, err ? undefined : this._getTemperatureDisplayUnits(dp)));
        })
        .on('set', this.setTemperatureDisplayUnits.bind(this));

      // Внутренний вентилятор кондиционера (если не скрыли)
      if (!this.device.context.hideIndoorFan && !this.device.context.noRotationSpeed) {
        ...
        if (this.accessory.addLinkedService) this.accessory.addLinkedService(acFan);
    ...
    // ===== HeaterCooler UI =====
      ...
      const service = this.accessory.getService(this.hap.Service.HeaterCooler)
        || this.accessory.addService(this.hap.Service.HeaterCooler, this.device.context.name);
      this._checkServiceName(service, this.device.context.name);
  ...
  _hkCurrentFromDps(dps) {
    const { Characteristic } = this.hap;
    // Thermostat.CurrentHeatingCoolingState: OFF(0), HEAT(1), COOL(2)
  ...
  getTemperatureDisplayUnits(callback) {
    const { Characteristic } = this.hap;
    if (!this.dpTempUnits) return callback(null, Characteristic.TemperatureDisplayUnits.CELSIUS);
  ...
  setTemperatureDisplayUnits(value, callback) {
    const { Characteristic } = this.hap;
    if (!this.dpTempUnits) return callback(); // нечего писать — молча ок
    this.setState(this.dpTempUnits, value === Characteristic.TemperatureDisplayUnits.FAHRENHEIT ? 'F' : 'C', callback);
  }

  // ==== Вентилятор кондиционера (числовой режим совместимости) ====
  _rsGetPercent(dps) {
  ...
  // ==== Приточка: Fanv2 + TargetFanState (AUTO/MANUAL) + сегменты скорости ====
  _setupFreshAirFanSegmented() {
    ...
    if (!fresh) {
      const title = this.device.context.freshAirName || 'Приточка';
      fresh = this.accessory.addService(Service.Fanv2, title, 'FreshAir');
    }
    if (this.accessory.addLinkedService) this.accessory.addLinkedService(fresh);

    // Активность: всё кроме "off" считаем включённым
    ...
    fresh.getCharacteristic(Characteristic.RotationSpeed)
      ...
      .on('get', (cb) => this.getState(this.dpFreshAir, (e, dp) => {
        ...
        if (dp === 'auto') return cb(null, 0); // в авто — крутимся 0, управляет TargetFanState
      ...
      .on('set', (val, cb) => {
        this.getState(this.dpFreshAir, (e, dp) => {
          if (e) return cb(e);
          if (dp === 'auto') return cb(); // авто режим — скорость не трогаем
