...
class AirConditionerAccessory extends BaseAccessory {
  ...
  constructor(...props) {
    ...
    this.healthBitIndex = Number.isInteger(this.device.context.healthBitIndex) ? this.device.context.healthBitIndex : 5; // бит 5 по твоей схеме
  ...
  _registerCharacteristics(dps) {
    ...
    // гарантированно получаем/создаём нужный первичный сервис
  ...
  _hkCurrentFromDps(dps) {
    ...
    // AUTO / WIND / WET → решаем по температуре (для Dom будет нормально)
  ...
  _setupFreshAirAsSwitches() {
    ...
    // Важен порядок: Авто первым (как просил)
    const defs = [
  ...
  _setupHealthSwitch() {
    ...
    sw.getCharacteristic(Characteristic.On)
      .on('get', (cb) => this.getState(this.dpBoolCode, (e, str) => {
        if (e) return cb(e);
        const on = this._readBoolBit(str, this.healthBitIndex, 0);
        cb(null, !!on);
      }))
      .on('set', (val, cb) => {
        this.getState(this.dpBoolCode, (e, cur) => {
          if (e) return cb(e);
          const next = this._writeBoolBit(cur, this.healthBitIndex, !!val, 0);
    ...
    this.device.on('change', (changes) => {
      ...
      const on = this._readBoolBit(changes[this.dpBoolCode], this.healthBitIndex, 0);
      const ch = sw.getCharacteristic(Characteristic.On);
