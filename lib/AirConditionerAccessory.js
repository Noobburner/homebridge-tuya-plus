...
class AirConditionerAccessory extends BaseAccessory {
  ...
  constructor(...props) {
    ...
    // совместимость со старыми числовыми скоростями (если кто-то использует)
  ...
  _registerCharacteristics(dps) {
    ...
    if (this.useThermostatUI) {
      ...
      if (!this.device.context.hideIndoorFan && !this.device.context.noRotationSpeed) {
        ...
        if (!acFan) {
          acFan = this.accessory.addService(Service.Fanv2, `${this.device.context.name} — Внутренний вентилятор`, 'ACFan');
        }
  ...
  _hkTargetFromDps(dps) {
    ...
    // AUTO: решаем по текущей/целевой температуре
    if (dps[this.dpMode] === this.cmdAuto) {
      ...
      if (Number.isFinite(curr) && Number.isFinite(tgtHuman)) {
        const hysteresis = 0.3; // небольшой гистерезис
      ...
      // около цели — считаем «выключено»
      return Characteristic.CurrentHeatingCoolingState.OFF;
    }

    // неизвестный режим — безопасно OFF
    return Characteristic.CurrentHeatingCoolingState.OFF;
  }
  ...
  // ==== Вентилятор кондиционера (старый числовой режим совместимости) ====
  _rsGetPercent(dps) {
  ...
  _setupFreshAirFanSegmented() {
    ...
    fresh.getCharacteristic(Characteristic.TargetFanState)
      ...
      .on('set', (val, cb) => {
        if (val === Characteristic.TargetFanState.AUTO) {
          ...
          // остаёмся в текущем manual, если авто — ставим low по умолчанию
    ...
    fresh.getCharacteristic(Characteristic.RotationSpeed)
      ...
      .on('set', (val, cb) => {
        // Если авто — игнорируем изменения скорости
        this.getState(this.dpFreshAir, (e, dp) => {
          if (e) return cb(e);
