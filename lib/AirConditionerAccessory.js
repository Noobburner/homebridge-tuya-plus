...
class AirConditionerAccessory extends BaseAccessory {
  ...
  _registerCharacteristics(dps) {
    ...
    if (this.useThermostatUI) {
      ...
      if (!service) service = this.accessory.addService(Service.Thermostat, this.device.context.name);
      this._checkServiceName(service, this.device.context.name);

      // CurrentTemperature (DP3)
      ...
      // Вентилятор внутреннего блока как Fanv2 «AC Fan» (по желанию)
      if (!this.device.context.noRotationSpeed) {
        let acFan = this.accessory.getService('ACFan');
        if (!acFan) {
          acFan = this.accessory.addService(Service.Fanv2, `${this.device.context.name} Fan`, 'ACFan');
        }
      ...
      if (!service) service = this.accessory.addService(Service.HeaterCooler, this.device.context.name);
      this._checkServiceName(service, this.device.context.name);
      ...
      // ЕДИНСТВЕННЫЙ сетпоинт => только CoolingThreshold
  ...
  setTemperatureDisplayUnits(value, callback) {
    const { Characteristic } = this.hap;
    // если у девайса нет dpTempUnits, устройство проигнорирует — ок
  ...
  _setupFreshAirFan() {
    ...
    let freshAirService = this.accessory.getService('FreshAir');
    if (!freshAirService) {
      freshAirService = this.accessory.addService(Service.Fanv2, `${this.device.context.name} Fresh Air`, 'FreshAir');
    }
