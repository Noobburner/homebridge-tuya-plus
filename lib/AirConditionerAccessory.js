...
class AirConditionerAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        // ВАЖНО: у твоего устройства режим авто = "auto" (нижний регистр)
        this.cmdAuto = 'auto';
        ...
        // Раньше плагин насильно отключал AUTO — убираем это.
        // Если кому-то надо выключить авто, пусть делает это конфигом: "noAuto": true
        // this.device.context.noAuto = true;  // ← удалено

        // Подготовим ступени «числового» вентилятора (сохраняем совместимость)
    ...
    // ===== Делители для целевой температуры (dp2) =====
    ...
    setSwingMode(value, callback) {
        ...
        const {Characteristic} = this.hap;
        switch (value) {
            case Characteristic.SwingMode.SWING_ENABLED:  return this.setState(this.dpSwingMode, true, callback);
            case Characteristic.SwingMode.SWING_DISABLED: return this.setState(this.dpSwingMode, false, callback);
        }
        callback();
    }
    ...
    // === Rotation Speed: поддержка строкового enum и числовых ступеней ===
